...
def test_decode_pfm(pfm_file):
    """pure tensorflow way to decode pfm file"""
    disparity = tf.io.read_file(pfm_file)
    disparity = tf.strings.substr(disparity, 18, 540 * 960 * 4 + 18, unit='BYTE')
    disparity = tf.reshape(tf.io.decode_raw(disparity, out_type=tf.float32), [540, 960])
    disparity = tf.reverse(disparity, [0])
    tf.assert_equal(disparity, readPFM(pfm_file))
